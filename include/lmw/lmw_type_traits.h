...
namespace lmw::type_traits {
    ...
    LMW_CREATE_ADVANCED_MEMBER_CHECK(has_list_handler, handle_list,
                                     std::declval<::lmw::atom::vector>(), long());

    LMW_CREATE_ADVANCED_MEMBER_CHECK(has_raw_list_handler, handle_raw_list,
                                    std::declval<::lmw::atom::span>(), long());

    LMW_CREATE_ADVANCED_MEMBER_CHECK(has_dsp_handler, process, std::declval<double**>(),
                                     std::declval<double**>(), long(), long(), long());
    
    LMW_CREATE_ADVANCED_MEMBER_CHECK(has_prepare_function, prepare, double(), long());
    
    LMW_CREATE_ADVANCED_MEMBER_CHECK(has_construct_function, construct, std::declval<::lmw::atom::vector>());
