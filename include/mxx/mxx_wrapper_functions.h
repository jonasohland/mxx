...
MXX_ALWAYS_INLINE void wrapper_handle_bang_impl(c74::max::t_object* obj)
{
    if constexpr (type_traits::has_bang_handler<user_class>())
        get_wrapper<user_class>(obj)->object.handle_bang(c74::max::proxy_getinlet(obj));

#ifdef MXX_REQUIRE_BANG_HANDLER
    static_assert(
        type_traits::has_bang_handler<user_class>(), "Missing required handle_bang function on external class");
#endif
}

template <typename user_class>
MXX_ALWAYS_INLINE void wrapper_handle_int_impl(c74::max::t_object* obj, long n)
{
    if constexpr (type_traits::has_int_handler<user_class>())
        get_wrapper<user_class>(obj)->object.handle_int(n, c74::max::proxy_getinlet(obj));
    else if constexpr (type_traits::has_mxx_int_handler<user_class>())
        find_self<user_class>(obj)->mxx_handle_int(n, c74::max::proxy_getinlet(obj));

#ifdef MXX_REQUIRE_INT_HANDLER
    static_assert(
        mxx::type_traits::has_int_handler<user_class>(), "Missing required handle_int function on external class");
#endif
}

template <typename user_class>
MXX_ALWAYS_INLINE void wrapper_handle_float_impl(c74::max::t_object* obj, double n)
{
    if constexpr (type_traits::has_float_handler<user_class>())
        get_wrapper<user_class>(obj)->object.handle_float(n, c74::max::proxy_getinlet(obj));

#ifdef MXX_REQUIRE_FLOAT_HANDLER
    static_assert(
        mxx::type_traits::has_float_handler<user_class>(), "Missing required handle_float function on external class");
#endif
}

template <typename user_class>
MXX_ALWAYS_INLINE void wrapper_handle_list_impl(c74::max::t_object* obj, c74::max::t_symbol*, long ac,
                                                c74::max::t_atom* av)
{
    if constexpr (type_traits::has_list_handler<user_class>())
        get_wrapper<user_class>(obj)->object.handle_list(detail::to_atom_vector(av, ac), c74::max::proxy_getinlet(obj));

    if constexpr (type_traits::has_raw_list_handler<user_class>())
        get_wrapper<user_class>(obj)->object.handle_raw_list(detail::to_span(av, ac), c74::max::proxy_getinlet(obj));

#ifdef MXX_REQUIRE_LIST_HANDLER
    static_assert(type_traits::has_list_handler<user_class>(), "Missing required handle_list(atom::vector) function on "
                                                               "external class");
#endif
...
template <typename user_class>
MXX_ALWAYS_INLINE void wrapper_handle_any_msg_impl(c74::max::t_object* obj, c74::max::t_symbol* s, long ac,
                                                   c74::max::t_atom* av)
{
    if constexpr (type_traits::has_any_msg_handler<user_class>()) {

        get_wrapper<user_class>(obj)->object.handle_any_msg(
            s, detail::to_atom_vector(av, ac), c74::max::proxy_getinlet(obj));
    }
...
template <typename user_class>
MXX_ALWAYS_INLINE void wrapper_assist_impl(c74::max::t_object* x, void*, long io, long index, char* s)
{
...
template <typename user_class>
MXX_ALWAYS_INLINE void wrapper_inletinfo_impl(c74::max::t_object* x, void*, long index, char* t)
{
    auto wrapper = get_wrapper<user_class>(x);
...
template <typename user_class>
MXX_ALWAYS_INLINE long wrapper_inputchanged_impl(c74::max::t_object* x, long index, long chans)
{
    auto* wrapper = get_wrapper<user_class>(x);

    if (MXX_UNLIKELY(safe_size_t(index) >= wrapper->object.m_inlets.size()))
        return 0;
...
template <typename user_class>
MXX_ALWAYS_INLINE long wrapper_multichanneloutputs_impl(c74::max::t_object* x, long idx)
{
    auto* wrapper = get_wrapper<user_class>(x);

    if (MXX_UNLIKELY(safe_size_t(idx) >= wrapper->object.m_outlets.size()))
        return 1;
...
template <typename user_class>
MXX_ALWAYS_INLINE void wrapper_dsp64_setup(c74::max::t_object* x, c74::max::t_object* dspman, short* count,
                                           double srate, long vsize, long, c74::max::t_perfroutine64 r)
{
    auto* wrapper = get_wrapper<user_class>(x);
    ...
    for (; i < input_flows; ++i) {
        ...
        if (is_mc)
            wrapper->object.m_inlets[i]->signal_count(reinterpret_cast<long>(
                c74::max::object_method(dspman, sym::getnuminputchannels, wrapper,
                                        MXX_MSVC_IGNORE_POINTER_TRUNCATION(reinterpret_cast<void*>(i)))));
    }

    for (; i < input_flows + output_flows; ++i)
        wrapper->object.m_outlets[i - input_flows]->m_connections = *(count + i);
...
template <typename user_class>
MXX_ALWAYS_INLINE void wrapper_dsp64_perform_impl(c74::max::t_object* x, c74::max::t_object*, double** ins, long numins,
                                                  double** outs, long numouts, long frames, long, void*)
{
    if constexpr (mxx::type_traits::has_dsp_handler<user_class>()) {
        get_wrapper<user_class>(x)->object.process(ins, outs, numins, numouts, frames);
    ...
    static_assert(type_traits::has_dsp_handler<user_class>(), "Missing required perform function on external class");
#endif
}

template <typename user_class>
MXX_ALWAYS_INLINE void wrapper_dsp64_user_setup(c74::max::t_object* x, c74::max::t_object* dspman, short* count,
                                                double srate, long vsize, long)
{
    if constexpr (mxx::type_traits::has_setup_dsp_function<user_class>()) {
        ...
        c74::max::dsp_add64(dspman, x, wrapper->object.setup_dsp(srate, vsize), 0, nullptr);

        static_assert(!mxx::type_traits::has_dsp_handler<user_class>(),
                      "Cannot use process() and setup_dsp() function in same class");
    }
