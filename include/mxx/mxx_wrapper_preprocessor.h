...
@endcode
*/
#define MXX_PREPROCESSOR_CAT_3(one, two, three) MXX_PREPROCESSOR_CAT_3_IMPL(one, two, three)
...
#define MXX_CREATE_ADVANCED_MEMBER_CHECK(sname, fname, ...)                                                            \
    ...
    struct sname##_impl {                                                                                              \
        template <typename C>                                                                                          \
        static constexpr decltype(std::declval<C>().fname(__VA_ARGS__), bool()) test(int)                              \
        {                                                                                                              \
            return true;                                                                                               \
        }                                                                                                              \
                                                                                                                       \
...
#define MMX_CREATE_MEMBER_VARIABLE_CHECK(sname, mname)                                                                 \
    ...
    struct sname##_impl: std::false_type {                                                                             \
                                                                                                                       ...
                                                                                                                       \
    template <typename T>                                                                                              \
    struct sname##_impl<T, decltype((void) T::mname, void())>: std::true_type {                                        \
    };                                                                                                                 \
    template <typename user_class>                                                                                     \
    std::integral_constant<bool, sname##_impl<user_class>::value> sname;
/* -------------------------------------------------------------------------- */
...
#define MXX_STATIC_WARNING(cond, msg) MXX_STATIC_WARNING_IMPL(cond, msg, __COUNTER__)
...
#    define MXX_CTTI_DEBUG_SECTION(user_class)                                                                         \
        MXX_STATIC_WARNING(                                                                                            \
            !mxx::type_traits::has_bang_handler<user_class>(), MXX_DEBUG_MSG_PREFIX "bang handler enabled");           \
        MXX_STATIC_WARNING(!mxx::type_traits::has_construct_function<user_class>(),                                    \
                           MXX_DEBUG_MSG_PREFIX "construct function enabled");                                         \
        MXX_STATIC_WARNING(                                                                                            \
            !mxx::type_traits::has_int_handler<user_class>(), MXX_DEBUG_MSG_PREFIX "int handler enabled");             \
        MXX_STATIC_WARNING(                                                                                            \
            !mxx::type_traits::has_float_handler<user_class>(), MXX_DEBUG_MSG_PREFIX "float handler enabled");         \
        MXX_STATIC_WARNING(                                                                                            \
            !mxx::type_traits::has_list_handler<user_class>(), MXX_DEBUG_MSG_PREFIX "list handler enabled");           \
        MXX_STATIC_WARNING(                                                                                            \
            !mxx::type_traits::has_any_msg_handler<user_class>(), MXX_DEBUG_MSG_PREFIX "any message handler enabled"); \
        MXX_STATIC_WARNING(                                                                                            \
            !mxx::type_traits::has_raw_list_handler<user_class>(), MXX_DEBUG_MSG_PREFIX "raw list handler enabled");   \
        MXX_STATIC_WARNING(                                                                                            \
            !mxx::type_traits::has_dsp_handler<user_class>(), MXX_DEBUG_MSG_PREFIX "DSP perform routine enabled");     \
        MXX_STATIC_WARNING(!mxx::type_traits::has_input_changed_function<user_class>(),                                \
                           MXX_DEBUG_MSG_PREFIX "inputchanged handler enabled");                                       \
        MXX_STATIC_WARNING(                                                                                            \
            !mxx::type_traits::has_dblclick_handler<user_class>(), MXX_DEBUG_MSG_PREFIX "dblclick handler enabled");   \
        MXX_STATIC_WARNING(!mxx::type_traits::has_setup_dsp_function<user_class>(),                                    \
...
#define MXX_MAX_METHOD(methodname) reinterpret_cast<c74::max::method>(&methodname)

#define MXX_DECLARE_PERFROUTINE(name)                                                                                  \
    void name(c74::max::t_object*, c74::max::t_object*, double**, long, double**, long, long, long, void*);
...
#define MXX_WRAPPER_FUNCTION_IMPL(method, prefix) MXX_PREPROCESSOR_CAT(prefix, method)

#define MXX_WRAPPER_FUNCTION(method, classname)                                                                        \
    MXX_WRAPPER_FUNCTION_IMPL(method, MXX_PREPROCESSOR_CAT(mxx_ext_wrap_, classname))

#define MXX_WRAPPER_FUNCTION_DSP64_METHOD(identifier) MXX_WRAPPER_FUNCTION(_dsp64_method, identifier)
...
#define MXX_WRAPPER_FUNCTION_INLETINFO(identifier) MXX_WRAPPER_FUNCTION(_inletinfo, identifier)

#define MXX_WRAPPER_FUNCTION_DBLCLICK(identifier) MXX_WRAPPER_FUNCTION(_dblclick, identifier)

#define MXX_WRAPPER_FUNCTION_INPUTCHANGED(identifier) MXX_WRAPPER_FUNCTION(_inputchanged, identifier)

#define MXX_WRAPPER_FUNCTION_MULTICHANNELOUTPUTS(identifier) MXX_WRAPPER_FUNCTION(_multichanneloutputs, identifier)

#define MXX_WRAPPER_FUNCTION_BANG(identifier) MXX_WRAPPER_FUNCTION(_bang, identifier)

#define MXX_WRAPPER_FUNCTION_INT(identifier) MXX_WRAPPER_FUNCTION(_int, identifier)

#define MXX_WRAPPER_FUNCTION_FLOAT(identifier) MXX_WRAPPER_FUNCTION(_float, identifier)

#define MXX_WRAPPER_FUNCTION_LIST(identifier) MXX_WRAPPER_FUNCTION(_list, identifier)

#define MXX_WRAPPER_FUNCTION_ANY_MSG(identifier) MXX_WRAPPER_FUNCTION(_any, identifier)

#define MXX_WRAPPER_FUNCTION_NAMED_METHOD(identifier) MXX_WRAPPER_FUNCTION(_named_method, identifier)

#define MXX_WRAPPER_FUNCTION_CLASS_NEW(identifier) MXX_WRAPPER_FUNCTION(_class_new, identifier)
...
#define MXX_CREATE_INT_HANDLER_FUNCTION(identifier, classname)                                                         \
    void MXX_WRAPPER_FUNCTION_INT(identifier)(c74::max::t_object * obj, long n)                                        \
    {                                                                                                                  \
        mxx::wrapper_handle_int_impl<classname>(obj, n);                                                               \
    }

#define MXX_CREATE_FLOAT_HANDLER_FUNCTION(identifier, classname)                                                       \
    void MXX_WRAPPER_FUNCTION_FLOAT(identifier)(c74::max::t_object * obj, double n)                                    \
    {                                                                                                                  \
...
#define MXX_CREATE_LIST_HANDLER_FUNCTION(identifier, classname)                                                        \
    void MXX_WRAPPER_FUNCTION_LIST(identifier)(                                                                        \
        c74::max::t_object * obj, c74::max::t_symbol * s, long argc, c74::max::t_atom* argv)                           \
    {                                                                                                                  \
...
#define MXX_CREATE_ASSIST_FUNCTION(identifier, classname)                                                              \
    void MXX_WRAPPER_FUNCTION_ASSIST(identifier)(c74::max::t_object * x, void* b, long io, long index, char* s)        \
    {                                                                                                                  \
        mxx::wrapper_assist_impl<classname>(x, b, io, index, s);                                                       \
    }

#define MXX_CREATE_INLETINFO_FUNCTION(identifier, classname)                                                           \
    void MXX_WRAPPER_FUNCTION_INLETINFO(identifier)(c74::max::t_object * x, void* b, long index, char* t)              \
    {                                                                                                                  \
        mxx::wrapper_inletinfo_impl<classname>(x, b, index, t);                                                        \
    }

#define MXX_CREATE_INPUTCHANGED_FUNCTION(identifier, classname)                                                        \
    void MXX_WRAPPER_FUNCTION_INPUTCHANGED(identifier)(c74::max::t_object * x, long index, long chans)                 \
    {                                                                                                                  \
...
#define MXX_CREATE_MULTICHANNELOUTPUTS_FUNCTION(identifier, classname)                                                 \
    long MXX_WRAPPER_FUNCTION_MULTICHANNELOUTPUTS(identifier)(c74::max::t_object * x, long idx)                        \
    {                                                                                                                  \
        return mxx::wrapper_multichanneloutputs_impl<classname>(x, idx);                                               \
    }

#define MXX_CREATE_DSP_PERFORM_FUNCTION(identifier, classname)                                                         \
    void MXX_WRAPPER_FUNCTION_DSP64_PERFORM(identifier)(c74::max::t_object * x, c74::max::t_object * dsp64,            \
...
#define MXX_CREATE_DSP_METHOD_FUNCTION(identifier, classname)                                                          \
    void MXX_WRAPPER_FUNCTION_DSP64_METHOD(identifier)(c74::max::t_object * x, c74::max::t_object * dsp64,             \
                                                       short* count, double samplerate, long maxvectorsize,            \
                                                       long flags)                                                     \
    {                                                                                                                  \
        mxx::wrapper_dsp64_setup<classname>(                                                                           \
            x, dsp64, count, samplerate, maxvectorsize, flags, MXX_WRAPPER_FUNCTION_DSP64_PERFORM(identifier));        \
    }

#define MXX_CREATE_DSP_USER_METHOD_FUNCTION(identifier, classname)                                                     \
    void MXX_WRAPPER_FUNCTION_DSP64_USER_METHOD(identifier)(c74::max::t_object * x, c74::max::t_object * dsp64,        \
                                                            short* count, double samplerate, long maxvectorsize,       \
                                                            long flags)                                                \
    {                                                                                                                  \
...
#define MXX_USER_CLASS_MAXCLASS_SYMBOL(ident) MXX_PREPROCESSOR_CAT(mxx_ext_maxclass_, ident)

#define MXX_USER_CLASS_MAXCLASS_DECL(ident) static c74::max::t_class* MXX_USER_CLASS_MAXCLASS_SYMBOL(ident);

#define MXX_NEW_INSTANCE_FUNCTION_NAME(ident) MXX_PREPROCESSOR_CAT(mxx_ext_newinstance_impl_, ident)

#define MXX_NEW_INSTANCE_FUNCTION_DECL(ident, name, ac, av)                                                            \
    void* MXX_NEW_INSTANCE_FUNCTION_NAME(ident)(c74::max::t_symbol * name, long ac, c74::max::t_atom* av)

#define MXX_NEW_INSTANCE_FUNCTION(ident, user_class)                                                                   \
    MXX_NEW_INSTANCE_FUNCTION_DECL(ident, ext_name, ext_ac, ext_av)                                                    \
    {                                                                                                                  \
        return mxx::wrapper_object_new<user_class>(MXX_USER_CLASS_MAXCLASS_SYMBOL(ident), ext_name, ext_ac, ext_av);   \
    }

#define MXX_FREE_INSTANCE_FUNCTION_NAME(ident) MXX_PREPROCESSOR_CAT(mxx_ext_freeinstance_impl_, ident)
...
#define MXX_MAXCLASS_DEF_IMPL(ident, class, extname)                                                                   \
    MXX_USER_CLASS_MAXCLASS_SYMBOL(ident) = mxx::wrapper_class_new<class>(                                             \
        ...
        MXX_MAX_METHOD(MXX_WRAPPER_FUNCTION_NAMED_METHOD(ident)), MXX_MAX_METHOD(MXX_WRAPPER_FUNCTION_INT(ident)),     \
        MXX_MAX_METHOD(MXX_WRAPPER_FUNCTION_BANG(ident)), MXX_MAX_METHOD(MXX_WRAPPER_FUNCTION_FLOAT(ident)),           \
        MXX_MAX_METHOD(MXX_WRAPPER_FUNCTION_LIST(ident)), MXX_MAX_METHOD(MXX_WRAPPER_FUNCTION_ANY_MSG(ident)),         \
        MXX_MAX_METHOD(MXX_WRAPPER_FUNCTION_ASSIST(ident)), MXX_MAX_METHOD(MXX_WRAPPER_FUNCTION_INLETINFO(ident)),     \
...
#define MXX_EXTERNAL(class, identifier, extname)                                                                       \
    ...
    {                                                                                                                  \
        MXX_MAXCLASS_DEF_IMPL(mxx_universal_helper, mxx::detail::mxx_universal_helper, MXX_UNIVERSAL_HELPER)           \
