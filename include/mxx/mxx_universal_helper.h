...
{
    auto* obj = c74::max::object_new(symbol("box"), symbol(MXX_STRING_LITERAL(MXX_UNIVERSAL_HELPER_NAME)));
    if (obj)
...
using unique_t_object = std::unique_ptr<c74::max::t_object, detail::maxobj_deleter>;

template <typename user_class>
using unique_mxxobj = std::unique_ptr<user_class, detail::mxxobject_deleter<user_class>>;


template <typename user_class, typename... Args>
std::unique_ptr<user_class, detail::mxxobject_deleter<user_class>> mxxobj_new(symbol nspace, symbol classn,
                                                                              Args&&... args)
{
    if (auto* obj = c74::max::object_new(nspace, classn, std::forward<Args>(args)...))
        return std::unique_ptr<user_class, detail::mxxobject_deleter<user_class>>(find_self<user_class>(obj));
    else
        throw std::runtime_error(classn.str().append(" creation failed"));
}

template <typename... Args>
std::unique_ptr<c74::max::t_object, detail::maxobj_deleter> maxobj_new(symbol nspace, symbol classn, Args&&... args)
{
    if (auto* obj = c74::max::object_new(nspace, classn, std::forward<Args>(args)...))
        return std::unique_ptr<c74::max::t_object, detail::maxobj_deleter>(obj);
    else
        throw std::runtime_error(classn.str().append(" creation failed"));
}
...
{
    if (auto helper = mxxobj_new<detail::mxx_universal_helper>("box", MXX_STRING_LITERAL(MXX_UNIVERSAL_HELPER_NAME))) {
...
void runtest()
{
    auto ptr = mxxobj_new<detail::mxx_universal_helper>("box", "universal helper");
}
