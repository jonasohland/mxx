...
enum class style { none, text, toggle, enum_sym, enum_int, rect, font, file, time, color };

static inline std::unordered_map<style, symbol> style_syms {
    { style::text, "text" }, { style::toggle, "onoff" }, { style::enum_sym, "enum" }, { style::enum_int, "enumindex" },
    { style::rect, "rect" }, { style::font, "font" },    { style::file, "file" },     { style::color, "rgba" }
};
}    // namespace attribute
...
struct attr_select_base {
    using type = typename std::enable_if<std::is_integral<AttrType>::value || std::is_floating_point<AttrType>::value,
